function _testset_demo_large(data_demo_large, scale, precond)
    ...
    @testset "$precond" begin
        @test sum(abs.(diff(TVRegDiff(data_demo_large, 40, 1e-1, ε=1e-8, scale=scale, precond=precond)))) < 1e4  
        @test sum(abs.(diff(TVRegDiff(data_demo_large, 40, 1e-3, ε=1e-6, scale=scale, precond=precond)))) < 1e4  
    end
end